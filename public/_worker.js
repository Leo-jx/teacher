...
const CONTINUATION_THRESHOLD = 3000;

function isContentIncomplete(content) {
...
async function fetchAIWithContinuation(API_URL, API_KEY, API_SECRET, MODEL_ID, messages, maxTokens, temperature, abortSignal) {
    ...
    while (rounds <= MAX_CONTINUATION_ROUNDS) {
        ...
        const needsContinuation = 
            finishReason === 'length' || 
            (finishReason !== 'stop' && finishReason !== 'eos_token' && finishReason !== 'end_of_message') ||
            (isContentIncomplete(fullContent) && chunk.length > 50);

        if (!needsContinuation) {
            if (finishReason !== 'length' && lastChunkLength < CONTINUATION_THRESHOLD && isContentIncomplete(fullContent)) {
        ...
        currentMessages = [
            ...
            { role: 'user', content: '请继续完成你的回答，保持连贯性和完整性。' }
        ];
    }
...
async function handleConfigRequest(env) {
    return new Response(JSON.stringify({
        success: true,
        config: {
            websocket: { available: false, message: '当前服务不支持WebSocket，请使用HTTP方式' },
            database: { available: !!env.DB }
        }
    }), { headers: corsHeaders });
}

const CODE_ANALYSIS_PROMPT = `你是一位资深软件架构师。用户会提供一段代码，你需要对其进行深入的结构和逻辑分析。
请严格按照以下格式输出：

## 代码分析报告

### 1. 功能概述
简要描述代码的核心功能和用途，用2-3句话概括。
...
### 4. 数据流转路径
分析数据在代码中的流动路径和转换过程，说明输入数据如何经各步骤处理成为输出。

### 5. 核心算法 / 逻辑说明
说明代码中使用的核心算法或关键逻辑，必要时用伪代码辅助说明。
...
注意事项：1. 必须精确定位执行点；2. 区分错误严重程度（警告/建议）；3. 修复建议必须是可直接运行的代码；4. 如果代码没有问题，说明代码质量良好并给出优化建议`;
...
async function handleLearnRequest(request, env) {
    try {
        ...
        if (type === 'syntax' && (!language || !keyword)) {
            return new Response(JSON.stringify({ error: '请提供语言和关键字参数' }), { status: 400, headers: corsHeaders });
        }
...
说明如何避免类似错误的发生`;
        ...
        try {
            ...
            return new Response(JSON.stringify({ error: '请求超时或网络延迟，请稍后重试' }), { status: 504, headers: corsHeaders });
        }
        clearTimeout(timeout);
